...
/**
 * Clean up email subject by removing multiple RE: or FW: prefixes
 * and ensuring only one RE: is present if needed.
 */
export function cleanSubject(subject) {
  if (!subject) return ''
  
  // Check if it's a reply/forward
  const isReplyOrForward = /^(re:|fw:|fwd:)/i.test(subject)
  
  if (isReplyOrForward) {
    // Remove all RE:, FW:, FWD: prefixes at the start recursively
    let cleaned = subject
    let lastCleaned
    do {
      lastCleaned = cleaned
